(() => {
  ...
  function renderResult(){
    ...
    const unavailable=(plan.unavailableMeals||[]).length?`<div class="plan-warning">Для условий «${(plan.unavailableMeals||[]).map(m=>MEALS[m][0].toLowerCase()).join(', ')}» подходящих рецептов пока нет. Ограничения не нарушены — измените время, технику или исключения.</div>`:'';
    $('#planTab').innerHTML=unavailable+DAYS.map((day,i)=>{const es=plan.entries.filter(x=>x.day===i);return `<section class="day-card"><header class="day-head"><h2>${day}</h2><small>${money(es.reduce((n,x)=>n+x.stats.cost,0))}</small></header>${es.map(e=>`<button class="meal-row" data-dish="${e.dish.id}"><span class="meal-icon">${MEALS[e.meal][1]}</span><span><small>${MEALS[e.meal][0]}</small><b>${e.dish.name}</b><small>⏱ ${e.dish.activeTime||e.dish.time} мин активно${e.dish.time>(e.dish.activeTime||e.dish.time)?` · ${e.dish.time} мин всего`:''}${profile.batchCooking&&e.dish.batch?' · на два дня':''}</small></span><span class="meal-price">${money(e.stats.cost)}</span></button>`).join('')}</section>`}).join('');
  ...
  function openRecipe(id){
    ...
    const s=d.source,sourceBox=$('#recipeSource');sourceBox.replaceChildren();
    if(s){const author=document.createElement('div');author.className='src-author';author.textContent=`👤 ${s.authorName}`;if(s.verified){const check=document.createElement('span');check.className='src-check';check.textContent=' ✓ проверено';author.append(check)}const title=document.createElement('div');title.className='src-title';title.textContent=`«${s.title}»`;const method=document.createElement('div');method.className='src-method';method.textContent=`${s.verificationMethod==='transcript'?'По расшифровке видео':'По официальному каналу с граммовками'} · исходный рецепт на ${s.sourceServings} порц.`;sourceBox.append(author,title,method);if(E.validSourceUrl(s.url)){const link=document.createElement('a');link.href=s.url;link.target='_blank';link.rel='noopener noreferrer';link.textContent='Открыть оригинал ↗';sourceBox.append(link)}}
    $('#recipeMeta').innerHTML=`<span>⏱ ${d.activeTime||d.time} мин активно${d.time>(d.activeTime||d.time)?` · ${d.time} мин всего`:''}</span><span>≈ ${money(stats.cost)}</span><span>${Math.round(stats.kcal/E.portions(profile))} ккал/порция</span><span>Б ${Math.round(stats.protein/E.portions(profile))} · Ж ${Math.round(stats.fat/E.portions(profile))} · У ${Math.round(stats.carbs/E.portions(profile))}</span>`;
    $('#recipeIngredients').innerHTML=d.ingredients.map(x=>{const q=x.g*scale,adjusted=q>=1000?`${(q/1000).toFixed(1)} кг`:`${Math.round(q)} г`;const original=Math.abs(scale-1)>0.01&&x.sourceAmount?`<small>у автора: ${x.sourceAmount}</small>`:'';return `<li>${E.product(x.p).name} — ${x.estimated?'≈ ':''}${adjusted}${original}</li>`}).join('');
    $('#recipeSteps').innerHTML=d.steps.map(x=>`<li>${x}</li>`).join('');
    $('#recipeSeasoning').innerHTML=(d.seasoning&&d.seasoning.length)?`<h3>Приправы и мелкие ингредиенты</h3><ul>${d.seasoning.map(x=>`<li>${x}</li>`).join('')}</ul>`:'';
    $('#recipeDialog').showModal();pulse();
  }
